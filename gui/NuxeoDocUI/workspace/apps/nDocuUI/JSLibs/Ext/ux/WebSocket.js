...
Ext.define('Ext.ux.WebSocket', {
    ...
    subscriptionMap: Ext.create('Ext.util.HashMap'),
    ...
    disconnect: function() {
        var me = this;
        me.ws.disconnect(me.onDisconnect.bind(me));
    },
     ...
     */
    send: function (queue, header, message) {

        var me = this;

        if (me.isReady()) {
            me.ws.send(queue, header, message);
        }
        else if (me.getKeepUnsentMessages()) {
            console.log('keeping unset msg');
            me.messageQueue.push(arguments);
        }

        return me;
    },

    subscribe: function(queue, callback, headers) {
        ...
        try{
            ...
            if(null != arguments.callee.caller && !Ext.isEmpty(arguments.callee.caller.$owner)){
                callee = arguments.callee.caller.$owner.$className;        
            }
            else{
                callee = arguments.callee.$owner.$className;   
            }
        }
        catch(err){
            Ext.Error.raise(err);
        ...
        if (me.isReady()) {
            return me.ws.subscribe(queue, callback, headers);
        }

        if (!subscriptionMap.containsKey(callee)) {

            var calleeMap = Ext.create('Ext.util.MixedCollection');
        }
    },
        ...
        if (me.getAutoReconnect() && (typeof me.autoReconnectTask === 'undefined')) {
            me.autoReconnectTask = Ext.TaskManager.start({
                run: function () {
                    ...
                    me.incReconnectCount();

                    console.log('Attempting to recoonect STATUS : ' + me.getStatus() + ' reconnectCount ' + me.getReconnectCount());
                    // It reconnects only if it's disconnected
                    if (me.getStatus() === me.CLOSED) {

                        me.initWebsocket(me.uid, me.pwd);
                    }
                   // me.updateReconnectCount();
                },
    ...
    incReconnectCount: function() {
        var count = this.getReconnectCount() + 1;
        this.setReconnectCount(count);
    },

    initWebsocket: function (uid, pwd) {
        var me = this;

        if (Ext.isEmpty(uid)) {

            if (Ext.isEmpty(this.getUserName())) {

                console.log("NULL UID / PWD");
            }
            else {
        ...
        me.ws = Ext.isEmpty(me.getProtocol()) ? Stomp.client(me.getUrl()) : Stomp.client(me.getUrl(), me.getProtocol());

        me.ws.connect(uid, pwd, me.onConnect.bind(me), me.onClose.bind(me));

        me.ws.debug = function(msg) {
            //console.log("debug " + msg);
        };

        me.ws.onopenX = function (evt) {
            // Kills the auto reconnect task
            // It will be reactivated at the next onclose event
            if (me.autoReconnectTask) {
                Ext.TaskManager.stop(me.autoReconnectTask);
        ...
        me.ws.onerrorX = function (error) {
            me.fireEvent('error', me, error);
        };

        me.ws.oncloseX = function (evt) {
            me.fireEvent('close', me);
    ...
    safeSend: function (data) {
        ...
        /*if (me.isReady()) me.ws.send(data);
            ...
            console.log("saving msg to queue");
            me.messageQueue.push(data);
        }*/

        if (me.isReady())
            me.ws.send(queue, header, message);
        else if (me.getKeepUnsentMessages()) me.messageQueue.push(arguments);
