/****** Object:  StoredProcedure [dbo].[UPD_user]    Script Date: 18/12/2019 12:31:45 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_user', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_user] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_user.sp', @g_id_line_num = 145
    ...
    IF(@is_disabled IS NOT NULL AND @is_disabled = 1)
    BEGIN
    	SELECT @is_allow_login = 0
    END
    ELSE IF(@is_disabled IS NOT NULL AND @is_disabled = 0 AND @is_allow_login IS NULL) 
    BEGIN
    	SELECT @is_allow_login = 1
    END
	...
	END
		...
		UPDATE	T_USER
		SET		id_user_ver				= id_user_ver + 1
				, --  id_user_ver				= @id_user_ver + 1
				...
				, id_region_key			= ISNULL(@id_region_key			, id_region_key)
				...
				, id_created_by_key		= ISNULL(@id_created_by_key		, id_created_by_key)
				, tx_email				= ISNULL(@tx_email				, tx_email)

		WHERE	id_user_key	= @id_user_key
		AND		id_user_ver		= @id_user_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> T_USER ' + '@id_user_key [' + CONVERT(varchar, @id_user_key) + '] '
		...
		SELECT @id_user_ver = @id_user_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_user] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_user.sp', @g_id_line_num = 403
