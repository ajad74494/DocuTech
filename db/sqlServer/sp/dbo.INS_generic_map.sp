...
/****** Object:  StoredProcedure [dbo].[INS_generic_map]    Script Date: 18/12/2019 12:39:15 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

	CREATE PROC [dbo].[INS_generic_map]
	...
	, @id_from_key			int
	, @id_from_key_ver		int						= 0
	, @id_from_type_key		int
	, @id_to_key			int
	, @id_to_key_ver		int						= 0
	, @id_to_type_key		int
	, @is_primary			int						= 0
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_generic_map', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_generic_map] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_generic_map.sp', @g_id_line_num = 140
	...
	END
		...
		(
			...
			, ISNULL(@id_from_key			, -2147483648)
			, ISNULL(@id_from_key_ver		, -2147483648)
			, ISNULL(@id_from_type_key		, -2147483648)
			, ISNULL(@id_to_key				, -2147483648)
			, ISNULL(@id_to_key_ver			, -2147483648)
			, ISNULL(@id_to_type_key		, -2147483648)
			, ISNULL(@is_primary			, 0)
		...
		SELECT @g_tx_err_msg = 'Error encountered while inserting -> T_GENERIC_MAP' + '@id_generic_map_key [' + CONVERT(varchar, @id_generic_map_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_generic_map] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_generic_map.sp', @g_id_line_num = 451
