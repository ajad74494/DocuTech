...
/****** Object:  StoredProcedure [nDOCUTECH].[UPD_customer]    Script Date: 9/11/2019 6:11:07 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[UPD_customer]
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_customer', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_customer] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_customer.sp', @g_id_line_num = 147
	...
	END
		...
		UPDATE	nDOCUTECH.T_CUSTOMER	
		SET		  id_customer_ver				= id_customer_ver + 1  
				...
				, tx_customer_number 		= ISNULL(@tx_customer_number 	, tx_customer_number)
				, tx_contact_number 		= ISNULL(@tx_contact_number 	, tx_contact_number)
				, tx_customer_order_no		= ISNULL(@tx_customer_order_no	, tx_customer_order_no)
				, tx_customer_address 		= ISNULL(@tx_customer_address 	, tx_customer_address)

		WHERE	id_customer_key	= @id_customer_key
		AND		id_customer_ver		= @id_customer_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> nDOCUTECH.T_CUSTOMER ' + '@id_customer_key [' + CONVERT(varchar, @id_customer_key) + '] '
		...
		SELECT @id_customer_ver = @id_customer_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_customer] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_customer.sp', @g_id_line_num = 386
