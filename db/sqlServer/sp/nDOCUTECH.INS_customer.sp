...
/****** Object:  StoredProcedure [nDOCUTECH].[INS_customer]    Script Date: 9/11/2019 6:09:54 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[INS_customer]
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_customer', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_customer] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_customer.sp', @g_id_line_num = 142
	...
	EXEC @g_id_return_status = GET_system_key @id_env_key	= @g_id_env_key, @tx_key_name = 'id_customer_key', @id_key_value	= @id_customer_key OUTPUT, @num_keys = 1

	SELECT @g_tx_err_msg = 'Error generating key for id_customer_key'
	IF (@g_id_return_status != 0)
	BEGIN
		RAISERROR (@g_tx_err_msg, 16, 1)
		IF (@g_is_sp_txn = 1)
    ...
    IF ( ((@id_customer_ver IS NULL) OR (@id_customer_ver = -2147483648)) )
	BEGIN
		SELECT @id_customer_ver = 0
	END
	...
	END
		...
		INSERT INTO nDOCUTECH.T_CUSTOMER
		(
		...
		(
			...
			, ISNULL(@id_doc_key				, 0			) 		
			, ISNULL(@tx_customer_number 		, '?'		)
			, ISNULL(@tx_contact_number 	  	, '?'		)
			, ISNULL(@tx_customer_order_no		, '?'		)	
			, ISNULL(@tx_customer_address 		, '?'		)

		)

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while inserting -> nDOCUTECH.T_CUSTOMER' + '@id_customer_key [' + CONVERT(varchar, @id_customer_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_customer] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_customer.sp', @g_id_line_num = 450
