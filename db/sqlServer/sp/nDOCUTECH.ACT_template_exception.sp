...
/****** Object:  StoredProcedure [nDOCUTECH].[ACT_template_exception]    Script Date: 9/11/2019 5:47:36 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[ACT_template_exception]
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='ACT_template_exception', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[ACT_template_exception] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_template_exception.sp', @g_id_line_num = 169
	...
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_template_exception.sp', @g_id_line_num = 326
	...
	BEGIN
		...
		END
			...
			EXEC @g_id_return_status = nDOCUTECH.INS_template_exception
										...
										, @tx_json_log_msg		= @tx_json_log_msg			OUTPUT
								  		...
								  		, @id_template_exception_key = @id_template_exception_key  	OUTPUT		
										, @id_template_exception_ver = @id_template_exception_ver 	OUTPUT		
										, @id_template_key 			 = @id_template_key 			OUTPUT
										, @tx_error_issue 			 = @tx_error_issue 		
										, @tx_error_issue_source	 = @tx_error_issue_source
										
										
			SELECT @g_tx_err_msg = 'Error calling SP -> [INS_template_exception] '
			IF (@g_id_return_status != 0)
			BEGIN
				RAISERROR (@g_tx_err_msg, 16, 1)
				IF (@g_is_sp_txn = 1)
	...
	BEGIN
		--BANK
		IF (((@id_template_exception_key IS NULL) OR (@id_template_exception_key = -2147483648)))
		BEGIN
			RAISERROR ('Error:Int [@id_template_exception_key] should not be NULL', 16, 1)
			IF (@g_is_sp_txn = 1)
				BEGIN
					SELECT @g_is_sp_txn = 0
					ROLLBACK TRANSACTION
		...
		BEGIN
			...
			BEGIN TRANSACTION
		END
			EXEC @g_id_return_status = nDOCUTECH.UPD_template_exception
										  @tx_action_name		= @tx_action_name
										...
										, @tx_error_issue 			 = @tx_error_issue 		
										, @tx_error_issue_source	 = @tx_error_issue_source
			SELECT @g_tx_err_msg = 'Error calling SP -> [UPD_template_exception] '
			IF (@g_id_return_status != 0)
			BEGIN
				RAISERROR (@g_tx_err_msg, 16, 1)
				IF (@g_is_sp_txn = 1)
	...
	BEGIN
		EXEC @g_id_return_status	= nDOCUTECH.SEL_template_exception
			 @tx_action_name		= @tx_action_name
	END
	...
	SELECT @g_tx_err_msg = 'Exiting[ACT_template_exception] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_template_exception.sp', @g_id_line_num = 1284
