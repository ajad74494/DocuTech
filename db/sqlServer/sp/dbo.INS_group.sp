...
/****** Object:  StoredProcedure [dbo].[INS_group]    Script Date: 18/12/2019 12:42:36 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_group', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_group] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_group.sp', @g_id_line_num = 135
	...
	END
		...
		(
			...
			, ISNULL(@tx_group_name				, '?')
			, ISNULL(@id_group_type_value_key 	, -2147483648)
			, ISNULL(@tx_desc					, '?')
		)

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while inserting -> T_GROUP' + '@id_group_key [' + CONVERT(varchar, @id_group_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_group] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_group.sp', @g_id_line_num = 429
