...
/****** Object:  StoredProcedure [dbo].[UPD_generic_map]    Script Date: 18/12/2019 12:40:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

	CREATE PROC [dbo].[UPD_generic_map]
	...
	, @id_from_type_key		int 				= NULL
	, @id_to_key			int 				= NULL
	, @id_to_key_ver		int 				= NULL
	, @id_to_type_key		int 				= NULL
	, @is_primary			int					= NULL
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_generic_map', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_generic_map] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_generic_map.sp', @g_id_line_num = 142
	...
	END
		...
		UPDATE	T_GENERIC_MAP
		SET		  id_generic_map_ver				= id_generic_map_ver + 1
				...
				, dtt_valid_from		= ISNULL(@dtt_valid_from	, dtt_valid_from)
				, dtt_valid_to			= ISNULL(@dtt_valid_to		, dtt_valid_to)
				, id_from_key			= ISNULL(@id_from_key			, id_from_key)
				, id_from_key_ver		= ISNULL(@id_from_key_ver		, id_from_key_ver)
				, id_from_type_key		= ISNULL(@id_from_type_key		, id_from_type_key)
				, id_to_key				= ISNULL(@id_to_key				, id_to_key)
				, id_to_key_ver			= ISNULL(@id_to_key_ver			, id_to_key_ver)
				, id_to_type_key		= ISNULL(@id_to_type_key		, id_to_type_key)
				, is_primary			= ISNULL(@is_primary			, is_primary)
				, ct_sort_order			= ISNULL(@ct_sort_order			, ct_sort_order)
				, tx_map_desc			= ISNULL(@tx_map_desc			, tx_map_desc)

		WHERE	id_generic_map_key	= @id_generic_map_key
		AND		id_generic_map_ver		= @id_generic_map_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> T_GENERIC_MAP ' + '@id_generic_map_key [' + CONVERT(varchar, @id_generic_map_key) + '] '
		...
		SELECT @id_generic_map_ver = @id_generic_map_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_generic_map] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_generic_map.sp', @g_id_line_num = 384
