...
/****** Object:  StoredProcedure [dbo].[UPD_preference]    Script Date: 18/12/2019 12:47:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

	CREATE PROC [dbo].[UPD_preference]
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_preference', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_preference] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_preference.sp', @g_id_line_num = 142
	...
	END
		...
		UPDATE	T_PREFERENCE
		SET		  id_pref_ver						= id_pref_ver + 1  
				, --  id_pref_ver				= @id_pref_ver + 1
				...
				, id_pref_type_value_key		= ISNULL( @id_pref_type_value_key			, id_pref_type_value_key )
				, id_pref_type_value_key_value 	= ISNULL( @id_pref_type_value_key_value 	, id_pref_type_value_key_value )
				, tx_pref_group					= ISNULL( @tx_pref_group					, tx_pref_group )
				, tx_pref_name					= ISNULL( @tx_pref_name						, tx_pref_name )
				, tx_pref_value					= ISNULL( @tx_pref_value					, tx_pref_value )
				, is_allow_override				= ISNULL( @is_allow_override				, is_allow_override )
				, int_pref_order				= ISNULL( @int_pref_order					, int_pref_order )
				, tx_pref_desc					= ISNULL( @tx_pref_desc						, tx_pref_desc )	
		WHERE	id_pref_key	= @id_pref_key
		AND		id_pref_ver		= @id_pref_ver
		
		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> T_PREFERENCE ' + '@id_pref_key [' + CONVERT(varchar, @id_pref_key) + '] '
		...
		SELECT @id_pref_ver = @id_pref_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_preference] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_preference.sp', @g_id_line_num = 378
