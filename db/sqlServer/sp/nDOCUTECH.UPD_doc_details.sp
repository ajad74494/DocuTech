...
/****** Object:  StoredProcedure [nDOCUTECH].[UPD_doc_details]    Script Date: 9/11/2019 6:11:07 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[UPD_doc_details]
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_doc_details', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_doc_details] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_doc_details.sp', @g_id_line_num = 147
	...
	END
		...
		UPDATE	nDOCUTECH.T_DOC_DETAILS
		SET		  id_doc_details_ver				= id_doc_details_ver + 1  
				, --  id_doc_details_ver				= @id_doc_details_ver + 1
				...
				, tx_item_name 					= ISNULL(@tx_item_name 				, tx_item_name 			)
				, tx_item_code 					= ISNULL(@tx_item_code 				, tx_item_code 			)
				, tx_item_description 			= ISNULL(@tx_item_description 		, tx_item_description 	)
				, part_no 					= ISNULL(@part_no 				, part_no 			)
				, item_qty 						= ISNULL(@item_qty 					, item_qty 				)
				, flt_unit_price 				= ISNULL(@flt_unit_price 			, flt_unit_price 		)
				, flt_total_price 				= ISNULL(@flt_total_price 			, flt_total_price 		)
				, flt_value_of_goods 			= ISNULL(@flt_value_of_goods 		, flt_value_of_goods 	)
				, tx_property_address 			= ISNULL(@tx_property_address 		, tx_property_address 	)
				, tx_rent 						= ISNULL(@tx_rent 					, tx_rent 				)
				, flt_insurance_premium 		= ISNULL(@flt_insurance_premium 	, flt_insurance_premium 	)
				, flt_net_value 				= ISNULL(@flt_net_value 			, flt_net_value 			)
				, reference_no 					= ISNULL(@reference_no 				, reference_no 			)
				, tx_pack 						= ISNULL(@tx_pack 					, tx_pack 				)
				, flt_trade 					= ISNULL(@flt_trade 				, flt_trade 				)
				, id_doc_key					= ISNULL(@id_doc_key				, id_doc_key				)

		WHERE	id_doc_details_key	= @id_doc_details_key
		AND		id_doc_details_ver		= @id_doc_details_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> nDOCUTECH.T_DOC_DETAILS ' + '@id_doc_details_key [' + CONVERT(varchar, @id_doc_details_key) + '] '
		...
		SELECT @id_doc_details_ver = @id_doc_details_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_doc_details] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_doc_details.sp', @g_id_line_num = 386
