USE [DOCUTECH_DEV]
GO
/****** Object:  StoredProcedure [nDOCUTECH].[INS_doc]    Script Date: 24/12/2019 10:03:02 ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	END
		...
		(			
			...
			, tx_trader_reference_no
			, id_template_key
			, is_sage50_processed		
			, tx_supplier_short_name						
		...
		(
			...
			, ISNULL(@supplier_number 				, 0			)
			, ISNULL(@delivery_note_no 				, 0			)
			, ISNULL(@receipt_number 				, 0			)
			, ISNULL(@document_number 				, 0			)
			, ISNULL(@dt_document					, GETDATE()	)
			, ISNULL(@recipient_of_invoice 			, 0			)
			, ISNULL(@vat_number 					, 0			)
			, ISNULL(@dt_despatch 		 			, GETDATE()	)
			, ISNULL(@tx_invoice_number 			, '?'		)
			, ISNULL(@tx_currency 					, '?'		)
			...
			, ISNULL(@flt_vat 						, 0		)
			, ISNULL(@flt_vat_rate 					, 0		)
			, ISNULL(@flt_vat_payable 				, 0		)
			, ISNULL(@flt_invoice_amount 			, 0		)
			, ISNULL(@flt_total_before_vat 			, 0		)
			, ISNULL(@flt_total_amount_due 			, 0		)
			, ISNULL(@flt_discount 					, 0		)
			, ISNULL(@flt_net_invoice_total			, 0		)
			, ISNULL(@tx_trader_reference_no		, 0			)
			, ISNULL(@id_template_key 				, 0			)
			, ISNULL(@is_sage50_processed			, 0)	
			, ISNULL(@tx_supplier_short_name 		, '?'		)
		
		)

		SELECT @id_doc_key = SCOPE_IDENTITY();
