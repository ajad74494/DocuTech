...
/****** Object:  StoredProcedure [nDOCUTECH].[ACT_customer]    Script Date: 9/11/2019 5:47:36 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[ACT_customer]
...
BEGIN
	...
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_customer.sp', @g_id_line_num = 169
	...
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_customer.sp', @g_id_line_num = 326
	...
	BEGIN
		...
		END
			...
			EXEC @g_id_return_status = nDOCUTECH.INS_customer
										...
										, @tx_customer_number 	= @tx_customer_number 
										, @tx_contact_number 	= @tx_contact_number 	
										, @tx_customer_order_no	= @tx_customer_order_no
										, @tx_customer_address 	= @tx_customer_address 
	...
	BEGIN
		--BANK
		IF (((@id_customer_key IS NULL) OR (@id_customer_key = -2147483648)))
		BEGIN
			RAISERROR ('Error:Int [@id_customer_key] should not be NULL', 16, 1)
			IF (@g_is_sp_txn = 1)
				BEGIN
					SELECT @g_is_sp_txn = 0
					ROLLBACK TRANSACTION
		...
		BEGIN
			...
			BEGIN TRANSACTION
		END
			EXEC @g_id_return_status = nDOCUTECH.UPD_customer
										  @tx_action_name		= @tx_action_name
	...
	BEGIN
		EXEC @g_id_return_status	= nDOCUTECH.SEL_customer
			 @tx_action_name		= @tx_action_name
	END
	...
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'ACT_customer.sp', @g_id_line_num = 1284
