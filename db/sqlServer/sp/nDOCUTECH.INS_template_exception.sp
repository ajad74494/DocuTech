...
/****** Object:  StoredProcedure [nDOCUTECH].[INS_template_exception]    Script Date: 9/11/2019 6:09:54 PM ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	CREATE PROC [nDOCUTECH].[INS_template_exception]
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_template_exception', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_template_exception] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_template_exception.sp', @g_id_line_num = 142
	...
	EXEC @g_id_return_status = GET_system_key @id_env_key	= @g_id_env_key, @tx_key_name = 'id_template_exception_key', @id_key_value	= @id_template_key OUTPUT, @num_keys = 1

	SELECT @g_tx_err_msg = 'Error generating key for id_template_exception_key'
	IF (@g_id_return_status != 0)
	BEGIN
		RAISERROR (@g_tx_err_msg, 16, 1)
		IF (@g_is_sp_txn = 1)
    ...
    IF ( ((@id_template_exception_ver IS NULL) OR (@id_template_exception_ver = -2147483648)) )
	BEGIN
		SELECT @id_template_exception_ver = 0
	END
	...
	END
		...
		INSERT INTO nDOCUTECH.T_TEMPLATE_EXCEPTION
		(
						
			  id_template_exception_key				
			, id_template_exception_ver					
			, -- id_template_exception_ver
			 is_active
			, id_env_key
			, id_user_mod_key
		...
		(
			  @id_template_exception_key
			, @id_template_exception_ver
			, --  ISNULL(@id_template_exception_ver			, 0)
			  ISNULL(@is_active						, 1			)
			, ISNULL(@id_env_key					, 0			)
			...
			, ISNULL(@id_template_key 			 	, 0			) 		
			, ISNULL(@tx_error_issue 		 		, '?'		)
			, ISNULL(@tx_error_issue_source		 	, '?'		)

		)

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error		
		SELECT @g_tx_err_msg = 'Error encountered while inserting -> nDOCUTECH.T_TEMPLATE_EXCEPTION' + '@id_template_key [' + CONVERT(varchar, @id_template_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_template_exception] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_template_exception.sp', @g_id_line_num = 450
