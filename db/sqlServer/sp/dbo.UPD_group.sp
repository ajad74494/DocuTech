...
/****** Object:  StoredProcedure [dbo].[UPD_group]    Script Date: 18/12/2019 12:43:52 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_group', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_group] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_group.sp', @g_id_line_num = 134
	...
	END
		...
		UPDATE	T_GROUP
		SET		  id_group_ver				= id_group_ver + 1
				, --  id_group_ver				= @id_group_ver + 1
				...
				, tx_group_name					= ISNULL(@tx_group_name		, tx_group_name)
				, id_group_type_value_key		= ISNULL(@id_group_type_value_key	, id_group_type_value_key)
		...
		SELECT @g_tx_err_msg = 'Error encountered while updating -> T_GROUP ' + '@id_group_key [' + CONVERT(varchar, @id_group_key) + '] '
		...
		SELECT @id_group_ver = @id_group_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_group] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_group.sp', @g_id_line_num = 368
